...
- `C:\sparring\Start-Sparring.cmd`는 1.3.1부터 레거시 진입점으로 제거 대상입니다. 새 설치/안내/검증은 EXE 바로가기와 `Sparring.Client.exe` 직접 실행 기준으로 합니다.
- StarCraft/SCHNAIL/Remastered 원본 설치 폴더는 읽기 전용 참조 대상입니다. 실행용 변경은 `C:\sparring\SC116AI`와 `C:\sparring\SC116AI_ai`에만 적용합니다.
...
- 코드 변경 시 기본 검증은 `dotnet test .\Sparring.sln -v:minimal`과 `.\scripts\smoke.ps1`입니다.
