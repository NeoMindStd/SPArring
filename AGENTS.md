...
- 사람 클라이언트는 기본 전체화면입니다. `내 클라이언트 전체화면` 체크가 켜져 있으면 W-MODE 플러그인을 끄고, 꺼져 있으면 사람 클라이언트도 창모드로 실행합니다.
- AI 클라이언트는 항상 창모드, 소리 OFF, APMAlert OFF입니다.
- APMAlert는 사람 클라이언트에만 선택적으로 켭니다. CoachAI를 되살리거나 봇/AI 클라이언트에 켜면 안 됩니다.
