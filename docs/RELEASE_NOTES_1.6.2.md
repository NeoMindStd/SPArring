...
봇/맵 선택 안정성과 게임 시작 후 AI 창 동작을 다듬은 핫픽스입니다.

## 주요 변경

- 게임 시작 후 AI 쪽 StarCraft 창이 다시 앞으로 튀어나오는 경우를 줄이고, 스파링 중에는 최소화 상태를 유지하도록 보강했습니다.
- Fighting Spirit 계열에서 드롭되거나 일꾼이 멈춘다고 제보된 봇/맵 조합은 기본 선택 후보에서 제외했습니다.
- 정상 동작하는 것으로 확인된 일부 봇/맵 조합은 다시 선택할 수 있게 복구했습니다.
- 런처와 설치 프로그램의 작은 창/큰 글꼴 환경에서 글자, 버튼, 목록이 겹치거나 잘리는 문제를 줄였습니다.
- 게임 종료 후 다음 실행 때 이전 종료 오류가 새 오류처럼 보이는 경우를 줄였습니다.
...
2. `Sparring-1.6.2-setup.exe`를 실행합니다.
