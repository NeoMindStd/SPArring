...
같은 Windows PC에서 Local PC LAN 방식의 독립 봇-vs-봇 경기를 여러 개 동시에 유지하는 것은 현재 검증 기준으로 삼지 않는다.

2026-07-03 재현 결과:

- 첫 번째 `Dragon` vs `Hannes Bredberg` 경기를 `(4)Fighting Spirit`에서 시작하면 양쪽 창이 인게임까지 정상 진입했다.
- 첫 경기가 인게임에 들어간 뒤 두 번째 경기를 시작하면, 두 번째 호스트는 방 생성 화면까지 들어가고 조인 쪽도 방 목록에서 해당 방을 볼 수 있었다.
- 그러나 조인 쪽에서 자동 참가가 되지 않았고, 수동으로 OK/Enter/더블클릭을 시도해도 `Unable to join game` 모달이 표시됐다.

따라서 한 PC에서 3경기를 병렬로 승률 측정하려는 검증은 별도 머신, 격리된 네트워크 환경, 또는 StarCraft Local PC 세션을 독립시킬 수 있는 별도 실행 구조가 준비될 때까지 보류한다. 현재 로컬 검증은 한 경기 단위로 끝까지 관찰하고 다음 경기를 시작하는 방식으로 수행한다.
