...
- 새 설치에서는 `C:\starai\Start-StarAI-PracticeClient.cmd`를 더 이상 생성하지 않습니다.
- 바탕화면/시작 메뉴 바로가기는 `StarAI.PracticeClient.App.exe`를 직접 실행합니다.
...
5. 설치가 끝나면 `StarAI Practice Client` 바로가기로 실행합니다.

## 보안 위협/차단 안내

StarAI는 StarCraft 1.16.1, BWAPI, 오래된 32비트 AI 봇 DLL/EXE를 함께 쓰기 때문에 Windows Defender, SmartScreen, 다른 백신 프로그램에서 오탐으로 막을 수 있습니다. 특히 처음 내려받은 설치 파일은 아직 많이 알려진 프로그램이 아니라서 SmartScreen이 “Windows의 PC 보호” 화면을 띄울 수 있고, 일부 봇 파일은 백신이 격리하면서 사라질 수 있습니다.

차단될 수 있는 파일 예시는 아래와 같습니다.

- `StarAI-PracticeClient-1.3.1-setup.exe`
- `StarAI.PracticeClient.App.exe`
- `C:\starai\SC116AI\Chaoslauncher - MultiInstance.exe`
- `C:\starai\SC116AI\Plugins\BWAPI_PluginInjector.bwl`
- `C:\starai\SC116AI\bwapi-data\BWAPI.dll`
- `C:\starai\SC116AI\bwapi-data\TM\TournamentModule.dll`
- `C:\starai\SC116AI\ddraw.dll`
- `C:\starai\SC116AI_ai\bwapi-data\AI\StarAI\Bots\...\*.dll`
...
1. 다운로드한 파일명이 `StarAI-PracticeClient-1.3.1-setup.exe`인지 확인합니다.
...
3. `보호 기록`을 누릅니다.
4. 방금 차단된 항목을 찾습니다. 파일 경로에 `C:\starai\StarAI.PracticeClient`, `C:\starai\SC116AI`, `C:\starai\SC116AI_ai`가 들어 있는지 확인합니다.
5. 항목을 펼친 뒤 `작업` 버튼이 있으면 `복원` 또는 `디바이스에서 허용`을 선택합니다.
6. 복원한 뒤 설치 프로그램을 다시 실행하거나, 이미 설치했다면 `StarAI Practice Client` 바로가기를 다시 실행합니다.

계속 같은 파일이 지워지는 경우에는 StarAI 폴더를 예외로 추가할 수 있습니다.
...
설치 경로를 바꿨다면 `C:\starai\StarAI.PracticeClient` 대신 직접 선택한 설치 폴더를 추가하면 됩니다.

주의할 점:

- 공식 릴리즈 페이지에서 받은 파일이 아닌 경우에는 예외 처리하지 말고 삭제하세요.
- 회사/학교 PC처럼 보안 설정이 관리되는 컴퓨터에서는 버튼이 비활성화될 수 있습니다. 이 경우 PC 관리자에게 문의해야 합니다.
- Windows 보안을 완전히 끄는 방식은 권장하지 않습니다. 필요한 StarAI 폴더만 예외로 추가하는 쪽이 안전합니다.
