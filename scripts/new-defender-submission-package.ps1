...
$zipPath = Join-Path $output.FullName "Sparring-$Version-defender-submission.zip"
...
elseif ($IncludeAllArtifacts) {
    foreach ($name in @(
        "Sparring-$Version-setup.exe",
        "Sparring-$Version-setup-folder.zip",
        "Sparring-$Version-win-x64.zip")) {
    ...
    $smallSetup = Join-Path $PSScriptRoot "..\artifacts\release\setup-folder-stage-$Version\Sparring-$Version-setup.exe"
    $defaultSetup = if (Test-Path -LiteralPath $smallSetup -PathType Leaf) {
        $smallSetup
    }
    else {
        Join-Path $dist "Sparring-$Version-setup.exe"
    }
...
Windows Defender 또는 SmartScreen이 Sparring 설치 파일이나 포함된 BWAPI/AI 봇 런타임 파일을 악성 또는 의심 파일로 탐지합니다.

## 설명

Sparring는 StarCraft 1.16.1 + BWAPI 기반 로컬 AI 스파링 런처입니다.
...
첨부 파일은 공식 GitHub 릴리즈에서 내려받은 Sparring 산출물입니다. 오탐 여부를 재분석해 주세요.
"@
