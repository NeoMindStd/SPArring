...
public sealed class NeoTerranBotCatalogTests
{
    [Fact]
    public void BundledCatalogIncludesNeoTerranFBwapiDll()
    {
        var repo = FindRepositoryRoot();
        var catalog = PracticeAssetCatalogReader.Read(Path.Combine(repo, "data"));

        var bot = Assert.Single(catalog.Bots, bot => bot.Name == "NeoTerranF");

        Assert.Equal(StarCraftRace.Terran, bot.Race);
        ...
        Assert.EndsWith("NeoTerranF", bot.SourceDirectory, StringComparison.OrdinalIgnoreCase);
        Assert.True(bot.HasSelectableBuilds);
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "factory_expand");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "two_factory_pressure");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "bio_academy");
        ...
        Assert.True(File.Exists(Path.Combine(bot.SourceDirectory!, "NeoTerranF.dll")));

        var bytes = File.ReadAllBytes(Path.Combine(bot.SourceDirectory!, "NeoTerranF.dll"));
    ...
    [Fact]
    public void NeoTerranFMapsStayOnVerifiedOneVsOneAndFourPlayerFamilies()
    {
        var repo = FindRepositoryRoot();
        var catalog = PracticeAssetCatalogReader.Read(Path.Combine(repo, "data"));
        var bot = Assert.Single(catalog.Bots, bot => bot.Name == "NeoTerranF");
        ...
        Assert.All(maps, map => Assert.True(IsNeoTerranFMapFamily(map), map.Name));
    }

    private static bool IsNeoTerranFMapFamily(PracticeMap map)
    {
