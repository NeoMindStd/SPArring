...
public sealed class SessionResultsTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "sparring-rating", Guid.NewGuid().ToString("N"), "rating.json");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "sparring-result-logs", Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "sparring-game-state", Guid.NewGuid().ToString("N"));
