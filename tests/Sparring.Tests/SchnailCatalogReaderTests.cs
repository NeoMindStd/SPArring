...
{
    [Fact]
    public void PracticeAssetCatalogReaderReadsBundledSparringAssetRoot()
    {
    ...
    [Fact]
    public void ReadFilesParsesMapCompatibilityAliases()
    {
        ...
        using var fixture = CatalogFixture.Create(
            ...
            [
              {
                "name": "(4)Polypoid 1.75",
                "fileName": "(4)Polypoid_1.75.scx",
                "guid": "{{updatedMapId}}",
                "compatibilityMapGuids": ["{{remasteredMapId}}", "{{originalMapId}}"],
                "enabled": true
              }
            ]
            """);
        ...
        var map = Assert.Single(catalog.Maps);
        Assert.Equal(updatedMapId, map.Id);
        Assert.Contains(originalMapId, map.EffectiveCompatibilityMapIds);
        Assert.Contains(remasteredMapId, map.EffectiveCompatibilityMapIds);
        Assert.DoesNotContain(updatedMapId, map.EffectiveCompatibilityMapIds);
    }
