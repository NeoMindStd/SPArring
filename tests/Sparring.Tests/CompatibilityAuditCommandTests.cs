...
public sealed class CompatibilityAuditCommandTests
{
    [Fact]
    public void LoadCatalogCanSkipConfiguredMapDirectories()
    {
        ...
        File.WriteAllText(Path.Combine(userMapRoot, "Injected Audit Custom.scx"), "map");
        ...
        var configuredCatalog = CompatibilityAuditCommand.LoadCatalog(paths, settings);
        Assert.Contains(configuredCatalog.Maps, map => map.Name == "Injected Audit Custom" && map.IsUserMap);
        Assert.Contains(configuredCatalog.Maps, map => map.Name.Contains("[Remastered Ladder]", StringComparison.Ordinal));

        var bundledCatalog = CompatibilityAuditCommand.LoadCatalog(paths, settings, includeConfiguredMaps: false);
        Assert.DoesNotContain(bundledCatalog.Maps, map => map.Name == "Injected Audit Custom");
        Assert.DoesNotContain(bundledCatalog.Maps, map => map.Name.Contains("[Remastered Ladder]", StringComparison.Ordinal));
    }

    [Fact]
    public void UsesBundledCatalogOnlyRecognizesCommandLineSwitch()
    {
        Assert.True(CompatibilityAuditCommand.UsesBundledCatalogOnly(["--audit-compatibility", "--bundled-catalog-only"]));
        Assert.False(CompatibilityAuditCommand.UsesBundledCatalogOnly(["--audit-compatibility"]));
    }
