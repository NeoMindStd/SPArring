...
public sealed class NeoZergBotCatalogTests
{
    [Fact]
    public void BundledCatalogIncludesNeoZergFBwapiDll()
    {
        var repo = FindRepositoryRoot();
        var catalog = PracticeAssetCatalogReader.Read(Path.Combine(repo, "data"));

        var bot = Assert.Single(catalog.Bots, bot => bot.Name == "NeoZergF");
        ...
        Assert.EndsWith("NeoZergF", bot.SourceDirectory, StringComparison.OrdinalIgnoreCase);
        Assert.True(bot.HasSelectableBuilds);
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "overpool_speed");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "two_hatch_muta");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "five_thirty_muta");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "three_hatch_hydra");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "five_hatch_hydra");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "nine_pool_speed");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Id == "lurker_contain");
        Assert.Contains(bot.AvailableBuildOptions, option => option.Matchups.Contains("vs Terran", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(bot.AvailableBuildOptions, option => option.Matchups.Contains("vs Protoss", StringComparison.OrdinalIgnoreCase));
        Assert.True(File.Exists(Path.Combine(bot.SourceDirectory!, "NeoZergF.dll")));

        var bytes = File.ReadAllBytes(Path.Combine(bot.SourceDirectory!, "NeoZergF.dll"));
    ...
    [Fact]
    public void NeoZergFMapsStayOnVerifiedOneVsOneAndFourPlayerFamilies()
    {
        var repo = FindRepositoryRoot();
        var catalog = PracticeAssetCatalogReader.Read(Path.Combine(repo, "data"));
        var bot = Assert.Single(catalog.Bots, bot => bot.Name == "NeoZergF");
        ...
        Assert.All(maps, map => Assert.True(IsNeoZergFMapFamily(map), map.Name));
    }

    private static bool IsNeoZergFMapFamily(PracticeMap map)
    {
