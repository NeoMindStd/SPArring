...
public sealed class InstallationVerifierTests
{
    ...
    public void VerifyPassesForUnchangedCopy()
    {
        ...
        File.WriteAllText(Path.Combine(source, "Sparring.Client.exe"), "app");
    ...
    public void RepairFromPayloadZipSkipsRunningExecutableWhenRequested()
    {
        ...
        var result = InstallationVerifier.RepairFromPayloadZip(
            ...
            payloadZip,
            issues,
            relativePath => !string.Equals(relativePath, "Sparring.Client.exe", StringComparison.OrdinalIgnoreCase));

        Assert.Equal(1, result.RequestedCount);
        Assert.Equal(0, result.RestoredCount);
        Assert.Equal("Sparring.Client.exe", Assert.Single(result.SkippedRelativePaths));
    }
    ...
    public void MissingRequiredRuntimeFilesReportsRelativePaths()
    {
        ...
        File.WriteAllText(Path.Combine(appRoot, "Sparring.Client.exe"), "app");
    ...
    private sealed class TempWorkspace : IDisposable
    {
        ...
        {
            var root = Path.Combine(Path.GetTempPath(), "sparring-install-verifier", Guid.NewGuid().ToString("N"));
