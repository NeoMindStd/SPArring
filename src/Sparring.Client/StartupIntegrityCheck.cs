...
internal static class StartupIntegrityCheck
{
    ...
    public static string FormatUserMessage(StartupIntegrityCheckReport report)
    {
        ...
        {
            lines.Add("Sparring 설치 파일 일부가 누락되었거나 손상되어 자동 복구했습니다.");
        }
        else
        {
            lines.Add("Sparring 설치 파일 일부를 복구하지 못했습니다.");
        }
        ...
        lines.Add("Windows 보안 > 바이러스 및 위협 방지 > 보호 기록에서 Sparring 관련 항목을 확인하고, 신뢰할 수 있는 공식 릴리즈 파일이면 복원 또는 허용을 선택하세요.");
        lines.Add(@"반복 차단되면 C:\sparring, C:\sparring\SC116AI, C:\sparring\SC116AI_ai 폴더를 Windows 보안 예외에 추가한 뒤 설치 프로그램을 다시 실행하세요.");
