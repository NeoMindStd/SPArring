...
public sealed partial class HotkeyCsvStore
{
    ...
    public string ImportFromDefaultAssets(PracticePaths paths, string runtimeRoot)
    {
        ...
        {
            throw new FileNotFoundException("Sparring bundled hotkey CSV was not found.", source);
        }
...
public sealed class HotkeyPatchApplier
{
    ...
    public static string BuildPatchedTbl(PracticePaths paths, IReadOnlyList<HotkeyEntry> entries)
    {
        ...
        {
            throw new FileNotFoundException("Sparring bundled stat_txt.txt was not found.", sourceStatText);
        }

        if (!File.Exists(compiler))
        {
            throw new FileNotFoundException("Sparring bundled TBL compiler was not found.", compiler);
        }

        var workDir = Path.Combine(Path.GetTempPath(), "Sparring", "hotkeys", Guid.NewGuid().ToString("N"));
    ...
    public static void InsertTblIntoRuntimeMpq(PracticePaths paths, string patchRtMpqPath, string statTxtTblPath)
    {
        ...
        var backup = patchRtMpqPath + ".sparring-hotkey-original";
        ...
        {
            throw new FileNotFoundException("Sparring bundled MPQ writer classpath was not found.", classPath);
        }
    ...
    {
        var helperDir = Path.Combine(Path.GetTempPath(), "Sparring", "mpq-helper");
        Directory.CreateDirectory(helperDir);
        var helperPath = Path.Combine(helperDir, "SparringMpqInsert.java");
    ...
    private const string JavaHelperSource = """
        ...
        public class SparringMpqInsert {
            public static void main(String[] args) throws Exception {
                if (args.length != 3) {
                    throw new IllegalArgumentException("Usage: SparringMpqInsert <mpq> <source-file> <target-name>");
                }
