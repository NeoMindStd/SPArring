...
namespace
{
    ...
    bool buildIdToOpening(const std::string& buildId, NeoZergE::Opening& opening)
    {
        const std::string id = trimLower(buildId);
        if (id == "overpool_speed")
        {
            opening = NeoZergE::Opening::OverpoolSpeed;
            return true;
        }
        if (id == "two_hatch_muta")
        {
            opening = NeoZergE::Opening::TwoHatchMuta;
            return true;
        }
        if (id == "five_thirty_muta")
        {
            opening = NeoZergE::Opening::FiveThirtyMuta;
            return true;
        }
        if (id == "three_hatch_hydra")
        {
            opening = NeoZergE::Opening::ThreeHatchHydra;
            return true;
        }
        if (id == "five_hatch_hydra")
        {
            opening = NeoZergE::Opening::FiveHatchHydra;
            return true;
        }
        if (id == "nine_pool_speed")
        {
            opening = NeoZergE::Opening::NinePoolSpeed;
            return true;
        }
        if (id == "lurker_contain")
        {
            opening = NeoZergE::Opening::LurkerContain;
            return true;
        }
...
void NeoZergE::onEnd(bool isWinner)
{
    if (!isWinner && !ggSent_)
    {
...
void NeoZergE::onSendText(std::string text)
{
    Broodwar->sendText("%s", text.c_str());
}

void NeoZergE::onReceiveText(Player, std::string)
{
}

void NeoZergE::onPlayerLeft(Player)
{
}

void NeoZergE::onNukeDetect(Position)
{
}

void NeoZergE::onUnitDiscover(Unit unit)
{
    updateEnemyStart(unit);
}

void NeoZergE::onUnitEvade(Unit)
{
}

void NeoZergE::onUnitShow(Unit unit)
{
    updateEnemyStart(unit);
}

void NeoZergE::onUnitHide(Unit)
{
}

void NeoZergE::onUnitCreate(Unit)
{
}

void NeoZergE::onUnitDestroy(Unit)
{
}

void NeoZergE::onUnitMorph(Unit unit)
{
    updateEnemyStart(unit);
}

void NeoZergE::onUnitRenegade(Unit)
{
}

void NeoZergE::onSaveGame(std::string)
{
}

void NeoZergE::onUnitComplete(Unit)
{
}

void NeoZergE::setInitialMapPositions()
{
    mainTile_ = Broodwar->self()->getStartLocation();
    naturalTile_ = TilePositions::Invalid;
}

void NeoZergE::chooseOpening()
{
...
void NeoZergE::updateEnemyStart(Unit unit)
{
...
bool NeoZergE::handleEmergencyDefense()
{
...
void NeoZergE::releaseDefenseDrones()
{
...
void NeoZergE::executeOverpoolSpeed()
{
...
void NeoZergE::executeTwoHatchMuta()
{
...
void NeoZergE::executeFiveThirtyMuta()
{
    if (supplyUsed() >= 9) ensureBuilding(UnitTypes::Zerg_Spawning_Pool, 1, mainTile_);
    if (supplyUsed() >= 10) ensureExtractor(1);
...
void NeoZergE::executeThreeHatchHydra()
{
    if (supplyUsed() >= 12 && allCount(UnitTypes::Zerg_Hatchery) < 2)
    {
        if (ensureExpansion(2) || supplyUsed() < 14)
...
void NeoZergE::executeFiveHatchHydra()
{
    if (supplyUsed() >= 12 && allCount(UnitTypes::Zerg_Hatchery) < 2)
    {
        if (ensureExpansion(2) || supplyUsed() < 14)
...
void NeoZergE::executeNinePoolSpeed()
{
    if (supplyUsed() >= 9) ensureBuilding(UnitTypes::Zerg_Spawning_Pool, 1, mainTile_);
    if (supplyUsed() >= 10) ensureExtractor(1);
    if (supplyUsed() >= 14) ensureExpansion(2);
...
void NeoZergE::executeLurkerContain()
{
    if (supplyUsed() >= 12 && allCount(UnitTypes::Zerg_Hatchery) < 2)
    {
        if (ensureExpansion(2) || supplyUsed() < 14)
...
void NeoZergE::manageProduction()
{
    ...
    {
        trainLarva(UnitTypes::Zerg_Mutalisk, 14);
    }

    if (hydraOpening && hasCompleted(UnitTypes::Zerg_Hydralisk_Den))
    {
        trainLarva(UnitTypes::Zerg_Hydralisk, opening_ == Opening::FiveHatchHydra ? 48 : 30);
    }

    const int zerglingCap = lingOpening ? 42 : (mutaOpening ? 8 : 16);
    ...
    {
        trainLarva(UnitTypes::Zerg_Hydralisk, 20);
    }
...
void NeoZergE::manageCombat()
{
    ...
    {
        shouldAttack = mutas >= 5 || armyCount() >= 18 || frame > 24 * 380;
    }
    else if (opening_ == Opening::ThreeHatchHydra || opening_ == Opening::FiveHatchHydra)
    {
        shouldAttack = hydras >= 8 || armyCount() >= 20 || frame > 24 * 340;
    }
    else if (opening_ == Opening::LurkerContain)
    {
        shouldAttack = lurkers >= 1 || hydras >= 10 || frame > 24 * 380;
    }
    else
    {
        shouldAttack = zerglings >= 8 || armyCount() >= 14 || frame > 24 * 290;
    }
...
int NeoZergE::supplyUsed() const
{
    return Broodwar->self()->supplyUsed() / 2;
}

int NeoZergE::supplyTotal() const
{
    return Broodwar->self()->supplyTotal() / 2;
}

int NeoZergE::allCount(UnitType type) const
{
...
int NeoZergE::completedCount(UnitType type) const
{
    int count = 0;
    for (auto unit : Broodwar->self()->getUnits())
    {
...
int NeoZergE::workerTarget() const
{
    ...
    return std::min(target, 64);
}

bool NeoZergE::hasOrMorphing(UnitType type, int count) const
{
    return allCount(type) >= count;
}

bool NeoZergE::hasCompleted(UnitType type, int count) const
{
    return completedCount(type) >= count;
}

bool NeoZergE::canAfford(UnitType type) const
{
...
bool NeoZergE::canAfford(UpgradeType type) const
{
    return Broodwar->self()->minerals() >= type.mineralPrice() &&
        Broodwar->self()->gas() >= type.gasPrice();
}

bool NeoZergE::canAfford(TechType type) const
{
    return Broodwar->self()->minerals() >= type.mineralPrice() &&
        Broodwar->self()->gas() >= type.gasPrice();
}

bool NeoZergE::ensureOverlord()
{
...
bool NeoZergE::ensureBuilding(UnitType type, int targetCount, TilePosition near, int maxRange)
{
...
bool NeoZergE::ensureExtractor(int targetCount)
{
...
bool NeoZergE::ensureExpansion(int targetCount)
{
...
bool NeoZergE::trainLarva(UnitType type, int maxCount)
{
...
bool NeoZergE::researchFromIdle(UnitType producer, TechType techType)
{
...
bool NeoZergE::upgradeFromIdle(UnitType producer, UpgradeType upgradeType)
{
...
bool NeoZergE::morphHydraliskToLurker(int maxCount)
{
...
Unit NeoZergE::pickWorker(Position near) const
{
    Unit best = nullptr;
    double bestDistance = std::numeric_limits<double>::max();
    for (auto unit : Broodwar->self()->getUnits())
...
Unit NeoZergE::nearestMineral(Position near) const
{
    Unit best = nullptr;
    double bestDistance = std::numeric_limits<double>::max();
...
Unit NeoZergE::nearestExtractorNeedingWorkers(Position near) const
{
    Unit best = nullptr;
    double bestDistance = std::numeric_limits<double>::max();
    for (auto unit : Broodwar->self()->getUnits())
...
Unit NeoZergE::nearestEnemyThreat(Position near, int radius) const
{
    Player enemy = Broodwar->enemy();
    if (!enemy)
    {
...
Unit NeoZergE::nearestEnemyTarget() const
{
    Player enemy = Broodwar->enemy();
    if (!enemy)
    {
...
int NeoZergE::combatUnitCountNear(Position near, int radius) const
{
    int count = 0;
    for (auto unit : Broodwar->self()->getUnits())
    {
...
int NeoZergE::enemyThreatCountNear(Position near, int radius, bool workersOnly) const
{
    Player enemy = Broodwar->enemy();
    if (!enemy)
    {
...
int NeoZergE::gasWorkersFor(Unit extractor) const
{
    if (!extractor)
    {
        return 0;
...
int NeoZergE::gasWorkerTarget(Unit extractor) const
{
...
TilePosition NeoZergE::findExpansionTile() const
{
...
Position NeoZergE::attackTarget() const
{
...
double NeoZergE::distance(Position a, Position b) const
{
...
std::string NeoZergE::configuredBuildId() const
{
    const char* candidates[] =
    {
        "bwapi-data\\AI\\Sparring\\Bots\\NeoZergE\\sparring-bot.ini",
...
std::string NeoZergE::openingName() const
{
    switch (opening_)
    {
    case Opening::OverpoolSpeed:
